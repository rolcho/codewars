...
export function stringToArray(str: string): number[] {
  ...
  const getArrayFromSlice = (s: string): number[] => {
    ...
    if (startAndIterationPlus !== s) {
      const { start, iteration, increment } = splitSlice(startAndIterationPlus, incrementNumber);
    ...
    if (startAndIterationMinus != s) {
      const { start, iteration, increment } = splitSlice(startAndIterationMinus, decrementNumber);
